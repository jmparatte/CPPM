...
volatile uint16_t
    CPPM_T_W;  // extended TCNT0 timer maintained by cycle function
volatile uint16_t
    CPPM_T_X;  // extended TCNT0 timer maintained by cycle function
volatile uint16_t CPPM_T_T;  // extended TCNT0 timer timeout

volatile bool CPPM_T_cycling;   // GPIOR0.0 = CPPM_T_cycle() in action, could be
                                // stopped by CPPM_T_interrupt()
...
void CPPM_T_check() {
  if (CPPM_T_checking && (uint8_t)(CPPM_T_timeout() >> 8) == 0)
    iservos_reset(true);
}
...
void oservos_setup() {
  ...
  CPPM.oservo = 0;
  for (int i = 0; i < CPPM_MSERVO; i++)
    CPPM.oservos[i] = CPPM_T_round(R615X_PULSE_CENTER);
...
ISR(TIMER1_CAPT_vect) {
  ...
  if (TCCR1B & (1 << ICES1))  // rising edge => end 300us synchro pulse ?
  {
    ...
    if (CPPM_T_syncing)  // ? follow a start edge
    {
      ...
      CPPM._sync2[CPPM.iservo] =
          CPPM.sync2;  // store sync width of current PWM servo pulse

      //			if(	CPPM.sync2 <
      //CPPM_T_floor(FRSKY_PULSE_SYNC-10) || 				CPPM.sync2 >
      //CPPM_T_ceil(FRSKY_PULSE_SYNC+50) ) // check sync width...
      //(FRSKY_PULSE_SYNC+20) is too short with R615X
      if (CPPM.sync2 < CPPM_PULSE_SYNC_MIN_FLOOR ||
          CPPM.sync2 > CPPM_PULSE_SYNC_MAX_CEIL) {
        iservos_reset(true);
      } else {
        if (CPPM.state == 0)
          CPPM.state = 1;  // 1st well formed sync pulse found.

        if (CPPM.state == 1)  // ? get pulse until gap pulse found
        {
          //					CPPM_T_T = tcnt0 +
          //CPPM_T_ceil(R615X_FRAME_NOTSYNC-FRSKY_PULSE_SYNC); // could be a
          //stange frame if wait so long !
          CPPM_T_T =
              CPPM.time5 + CPPM_FRAME_NOTSYNC_CEIL;  // could be a stange frame
                                                     // if wait so long !
        } else  // : gap pulse found => start time of frame known.
            if (CPPM.state == 2)  // ? get pulses and compute nservo.
        {
          //					CPPM_T_T = CPPM.time5 +
          //CPPM_T_ceil(R615X_FRAME_NOTSYNC); // 2% max oscillator error
          CPPM_T_T = CPPM.time5 + CPPM_FRAME_NOTSYNC_CEIL;  // set frame timeout
        } else  // : check pulses and gap.
        {
          if (CPPM.iservo < CPPM.nservo)
            //						CPPM_T_T = CPPM.time0 +
            //CPPM_T_ceil(R615X_PULSE_CENTER+R615X_PULSE_C150PC); // middle
            //stick+150% 						CPPM_T_T = CPPM.time0 +
            //CPPM_T_ceil(R615X_PULSE_CENTER+R615X_PULSE_C200PC); // middle
            //stick+200%
            CPPM_T_T = CPPM.time0 +
                       CPPM_PULSE_CENTER_PLUS_C200PC_CEIL;  // middle stick+200%
          else
            //						CPPM_T_T = CPPM.time5 +
            //CPPM_T_ceil(R615X_FRAME_NOTSYNC);
  ...
  } else  // : falling edge => start 300us sync pulse and start PWM servo pulse.
  {
    ...
    //		if (state==3) PORTB = (PORTB & ~CPPM_PBMASK) | (kservo & CPPM_PBMASK);
    //// update PWM channels, stop current pulse, start next pulse
    ...
    if (CPPM.state == 0) {
      CPPM.time5 = CPPM.time0 =
          tcnt0;  // set start time of next PWM servo pulse (and sync pulse)
    } else        // : state>=1
    {
      ...
      int puls3i = ((signed)CPPM.puls3 - R615X_PULSE_CENTER) /
                   4;  // middle centered servo pulse
                       //			int puls3i = (signed) CPPM.puls3 / 4 -
      //(CPPM.sync2 + CPPM.sync2/4); // middle centered servo pulse
      if (puls3i > 127)
        puls3i = 127;
      else if (puls3i < -128)
        puls3i = -128;
      ...
      //			if (CPPM.puls3 <
      //CPPM_T_floor(R615X_PULSE_CENTER-R615X_PULSE_C150PC)) // too short servo
      //pulse (middle stick-150%) ? 			if (CPPM.puls3 <
      //CPPM_T_floor(R615X_PULSE_CENTER-R615X_PULSE_C200PC)) // too short servo
      //pulse (middle stick-200%) ?
      if (CPPM.puls3 <
          CPPM_PULSE_CENTER_MINUS_C200PC_FLOOR)  // too short servo pulse
                                                 // (middle stick-200%) ?
      {
        iservos_reset(true);
      } else
          //			if (CPPM.puls3 >
          //CPPM_T_ceil(R615X_PULSE_CENTER+R615X_PULSE_C150PC)) // is a gap
          //pulse (middle stick+150%) ? 			if (CPPM.puls3 >
          //CPPM_T_ceil(R615X_PULSE_CENTER+R615X_PULSE_C200PC)) // is a gap
          //pulse (middle stick+200%) ?
          if (CPPM.puls3 >
              CPPM_PULSE_CENTER_PLUS_C200PC_CEIL)  // is a gap pulse (middle
                                                   // stick+200%) ?
      {
        CPPM.cppm4 =
            tcnt0 - CPPM.time5;  // compute length of elapsed CPPM frame
        ...
        //				if(	(CPPM.state==3 && CPPM.cppm4 <
        //CPPM_T_floor(R615X_FRAME_LENGTH)) || // frame length too short ?
        if ((CPPM.state == 3 &&
             CPPM.cppm4 <
                 CPPM_FRAME_LENGTH_FLOOR) ||  // frame length too short ?
            //					CPPM.cppm4 > CPPM_T_ceil(R615X_FRAME_NOTSYNC) ) // frame
            //length too long ?
            CPPM.cppm4 > CPPM_FRAME_NOTSYNC_CEIL)  // frame length too long ?
        {
          ...
          CPPM.jservo = 1;  // set next servo (2nd)
                            //					CPPM.kservo = CPPM.lservo[1]; // set mask of
                            //next servo
      ...
      } else  // valid servo pulse.
      {
        ...
        {
          iservos_reset(true);
        } else if (CPPM.jservo > CPPM.nservo)  // servos overflow ?
        {
          iservos_reset(true);
        } else if (CPPM.jservo == CPPM.nservo) {
          CPPM.jservo = 0;
          //					kservo = lservo[0]; // set next mask of 1st
          //servo

          CPPM._received = true;
        } else {
          CPPM.jservo++;
          //					kservo = lservo[jservo]; // set next mask of
          //next servo
        }
      }
    }
...
bool CPPM_Class::synchronized() { return state == 3; }
...
int CPPM_Class::read_us(int n) {
  return (CPPM_T_div * (long)read(n) + (CPPM_T_mul - 1) / 2) /
         CPPM_T_mul;  // round
}
...
CPPM_Class::operator bool() { return true; }
